...
def show_settings_ui(parent_window):
    ...
    # 读取当前模式 (0: 读秒, 1: Steam)
    current_mode = data.get("cha_chuan_1", {}).get("cha_chuan_1_style", 0)
    # 【新增】读取 手动 开关状态
    current_manual = data.get("cha_chuan_1", {}).get("manual", False)
    ...
    def set_mode(mode_val):
        ...
        """
        # 1. 更新配置文件
        cfg.update_config_item("cha_chuan_1.cha_chuan_1_style", mode_val)
    ...
    # 填入当前配置
    initial_time = data.get("cha_chuan_1", {}).get("cha_chuan_1_time", 40)
    ...
    def save_time(event=None):
        try:
            ...
            cfg.update_config_item("cha_chuan_1.cha_chuan_1_time", val)
            win.focus()  # 移除焦点以确认
        except:
            pass
    ...
    def set_manual():
        ...
        cfg.update_config_item("cha_chuan_1.manual", is_manual)
...
def get_style1():
    ...
    # 3. 提取 target_cmd1，使用 .get 防止报错（如果没找到返回空字符串）
    style = data.get("cha_chuan_1", {}).get("cha_chuan_1_style", 0)
...
def get_time1():
    ...
    # 3. 提取 target_cmd1，使用 .get 防止报错（如果没找到返回空字符串）
    time = data.get("cha_chuan_1", {}).get("cha_chuan_1_time", 40)
...
def manual():
    ...
    # 3. 提取 target_cmd1，使用 .get 防止报错（如果没找到返回空字符串）
    manual = data.get("cha_chuan_1", {}).get("manual", False)
