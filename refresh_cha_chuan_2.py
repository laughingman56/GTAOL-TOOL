...
def get_cmd2_id():
    ...
    # 🔍 添加这行，查看实际路径
    print(f"配置文件路径: {config.filename}")
    # 2. 获取所有配置数据
    ...
    # 方法2：正则表达式匹配数字串
    match = re.search(r'steam://rungame/\d+/(\d+)/', cmd2)
    if match:
        BOT_ID = match.group(1)
        print(f"grouid: {BOT_ID}")  # 输出: 76561198451940904
        return BOT_ID
...
def get_aim_style2():
    ...
    # 3. 提取 target_cmd，使用 .get 防止报错（如果没找到返回空字符串）
    aim_style2 = all_data.get("aim_style2", )
...
def get_latest_steam_link_from_log(id):
    """
        ...
        for i in range(len(lines) - 1, -1, -1):
            ...
            if "ExecuteSteamURL" in line:
                ...
                if match:
                    ...
                    if "rungame" in url and "steamjvp" in url and  id in url:
...
def on_read_key(id):
    link, msg = get_latest_steam_link_from_log(id)
    print(msg)
    ...
    if link:
        ...
        # 写入配置
        config_manager.update_config_item("target_cmd1", link)


def main():

    ID = get_cmd2_id()  # 传递实例
    aim_style = get_aim_style2()
